...
3. Review linked pull requests, issues, Linear work, related repositories in `usa-acc`, and relevant external-organization repositories whenever behavior or contracts cross boundaries.
...
For branch integration, avoid git rebase in favor of git merge so shared history remains append-only and reviewable.
